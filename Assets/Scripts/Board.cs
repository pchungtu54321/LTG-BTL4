...
#pragma warning restore 0649
    ...
    public void SetTiles(TileData[] tileDatas) {
        ...
        _tiles = new Tile[25];
        for(int i = 0; i < 25; i++) {
            if(tileDatas[i]._workerData != null) {
                Worker.Gender _tempGender = tileDatas[i]._workerData._gender;
                Worker.Colour _tempColour = tileDatas[i]._workerData._colour;
                string tempComp = _tempGender + "Worker" + _tempColour + "(Clone)";

                GameObject workerGameObject = GameObject.Find(tempComp);
                if (workerGameObject != null) {
                    Worker workerComponent = workerGameObject.GetComponent<Worker>();
                    workerComponent.SetTile(_tiles[i]);
                }
                // _tiles[i].PlaceWorker(newWorkerGO, gender, colour);
            }
            for(int j = 0; j < tileDatas[i]._towerPieceCount; j ++) {
                _tiles[i].AddTowerPiece();
                Debug.Log(i);
                Debug.Log("tower");
            }
        }
